...
def mseprom(y_true, y_pred, chrom, gene_annotations,
...
def parse_arguments():
    parser = argparse.ArgumentParser(prog='ENCODE Imputation Challenge scoring'
                                          'script.')
    ...
    parser.add_argument('--chrom', nargs='+', type=str, required=True,
                        help='List of chromosomes to score for '
                             '(e.g. chr3 chr21).')
    ...
    parser.add_argument('--out', default='output.tsv', type=str,
                        help='Output TSV file. (chrom, mse, mse1obs, '
                             'mse1imp, gwcorr, match1, catch1obs, '
                             'catch1imp, aucobs1, aucimp1, mseprom, '
                             'msegene, mseenh)')
...
def main():
    ...
    with open(args.out.strip("'"), 'w') as fp:
        for chrom in args.chrom:
            ...
            output = [chrom]
            for func in (mse, mse1obs, mse1imp, gwcorr, match1,
                         catch1obs, catch1imp, aucobs1, aucimp1):
                output.append(func(y_true, y_predicted))
            output.append(mseprom(y_true, y_predicted, chrom, gene_annotations,
                                  args.window_size, args.prom_loc))
            output.append(msegene(y_true, y_predicted, chrom, gene_annotations,
                                  args.window_size))
            output.append(mseenh(y_true, y_predicted, chrom, enh_annotations,
                                 args.window_size))
