...
for cell_assay, team_ids in RANK_PER_CELL_ASSAY.items():
    track_hub = copy.deepcopy(TRACK_HUB_BASE)
    with open(cell_assay + '.json', 'w') as fp:
        for k in [-1] + team_ids:
            ...
            track['name'] = '{name} ({id})'.format(
                id=k,
                name=TEAM_NAME[k])
            track['url'] = '{root}/{cell_assay}.bigwig'.format(
                root=URL_ROOT[k],
            ...
            track_hub.append(track)
        fp.write(json.dumps(track_hub, indent=4) + '\n')